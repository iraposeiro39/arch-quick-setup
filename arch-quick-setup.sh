...
sleep 1
echo "label: dos" | sfdisk /dev/sda &> /dev/null
## Making the Swap partition, formatting and mounting it
echo "Making the Swap partition..."
sleep 1
echo "size:"$SWAP"G" | sfdisk /dev/sda -N 1 &> /dev/null
echo "type: S" | sfdisk /dev/sda -N 1 &> /dev/null
mkswap /dev/sda1
swapon /dev/sda1
## Making the Standard partition, formatting and mounting it
echo "Making the Standard partition..."
sleep 1
echo "size:" "" | sfdisk /dev/sda -N 2 &> /dev/null
echo "type: L" | sfdisk /dev/sda -N 2 &> /dev/null
mkfs.ext4 /dev/sda2
mount /dev/sda2 /mnt
